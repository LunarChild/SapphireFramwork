...
namespace Sapphire.Service.Implements
{
    ...
    */
    public class CmsArticleService : BaseService<CmsArticle>, ICmsArticleService
    {
        ...
        /// <returns></returns>
        public async Task<ApiResult<string>> UpdateAsync(CmsArticle parm)
        {
            var res = new ApiResult<string>() { statusCode = (int)ApiEnum.Error };
            try
            {
                await Db.Updateable(parm).IgnoreColumns(m => new { m.LastHitDate, m.AddDate,m.DelDate }).ExecuteCommandAsync();
                res.statusCode = (int)ApiEnum.Status;
            }
            catch (Exception ex)
            {
