...
namespace Sapphire.Service.Implements
{
    ...
    */
    public class CmsColumnService : BaseService<CmsColumn>, ICmsColumnService
    {
        ...
        /// <returns></returns>
        public async Task<ApiResult<string>> AddAsync(CmsColumn parm)
        {
            ...
            try
            {
                ...
                //根据模板ID查询相关内容
                var mbModel =await Db.Queryable<CmsTemplate>().SingleAsync(m => m.Id == parm.TempId);
                if (mbModel != null)
                {
                ...
                var result = await Db.Ado.UseTranAsync(() =>
                {
                    ...
                    return Db.Updateable(parm).ExecuteCommandAsync();
                });
                if (result.IsSuccess)
                {
