layui.define(['layer', 'toastr', 'pjax'], function (exports) {
    ...
    var tmls,tool = {
        ...
        ajax: function (url, options, callFun,method='post') {
            var httpUrl = "/", token = tool.GetSession('SapphireADMIN_ACCESS_TOKEN');
            var _headers = {};
            if (token !== null) {
                _headers = {
            ...
            $.ajax(httpUrl + url, {
                ...
                success: function (data) {
                    if (data.statusCode == 408 || data.statusCode == 407) {
                        window.location.href = '/Sapphireadmin/login?ReturnUrl='+window.location.href;
                    } else {
                        callFun(data);
                    }
                },
        ...
        getToken: function () {
            var token = tool.GetSession('SapphireADMIN_ACCESS_TOKEN');
        ...
        cloudFile: function () {
            $(".Sapphire-cloud").click(function () {
                ...
                tool.Open('媒体资源库', '/Sapphireadmin/file/cloud/?type=' + type + '&img=' + showImg + '&control=' + input_text + frameUrl, '950px', '600px');
            });
        },
